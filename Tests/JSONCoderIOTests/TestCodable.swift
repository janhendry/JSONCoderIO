...
extension JSONCoderIOTests {
    ...
    func testCodable<T:Codable & BundelPath>(typ:T) {
        let path = T.path().subDic + "/" + T.path().file
        
        guard let url = Bundle.module.url(forResource:T.path().file, withExtension: T.path().ext,subdirectory: T.path().subDic)
    ...
    func testCodable<T:Codable & BundelPath>(_ typ:T.Type) {
